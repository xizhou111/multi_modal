...
from blip2_query_cls.blip2_qformer import Blip2Qformer
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if accelerator.is_main_process:
                ...
                if step % 5 == 0:
                    train_bar.write(f"Epoch {epoch}, Step {step}, lr: {optimizer.param_groups[0]['lr']}, loss: {loss_reduced.item()}, loss_itc: {loss_itc_reduced.item()}, loss_itm: {loss_itm_reduced.item()}")

                tf_writer.add_scalar("train/loss", loss_reduced, step)
                # tf_writer.add_scalar("train/loss_itc", loss_itc_reduced, step)
                # tf_writer.add_scalar("train/loss_itm", loss_itm_reduced, step)
                tf_writer.add_scalar("train/learning_rate", optimizer.param_groups[0]['lr'], step)
            ...
            if step % 2000 == 0:
                ...
                with torch.no_grad():
                    ...
                    for val_batch in tqdm(val_loader, desc="extracting val data features", disable=not accelerator.is_main_process):
                        ...
                        image_features = model.module.extract_features(val_batch, mode='image')['image_embeds_proj'][:,0,:].cpu()
                ...
                if accelerator.is_main_process:
                    for k, v in recalls_i2t.items():
                        tf_writer.add_scalar(f"val/i2t_recall@{k}", v, step)
